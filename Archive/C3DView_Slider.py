...
from utility.Geoms import Line, PointCloud, CoordSys
from utility import Transforms as tf
...
def main():
    ...
    ax = fig.add_axes([0.05, 0.2, 0.45, 0.7], projection='3d', proj_type='ortho')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ...
    # Create a PointClound class instance in order to display the markers
    pts_obj = PointCloud('[MARKERS]', ax)
    pts_obj.set_point_names(mkr_names)
    pts_obj.set_from_points(mkr_pts)
