...
def main():
    ...
    i_vect = (dict_mkr_coords['RVKN']-dict_mkr_coords['RVAN'])
    i_vect = i_vect / np.linalg.norm(i_vect, axis=1)[:, None]
    j_vect = np.cross(i_vect, dict_mkr_coords['RKNE']-dict_mkr_coords['RVKN'], axis=1)
    j_vect = j_vect / np.linalg.norm(j_vect, axis = 1)[:, None]
    k_vect = np.cross(i_vect, j_vect)
    jcs_r_shank = np.dstack((i_vect, j_vect, k_vect))

    i_vect = (dict_mkr_coords['RVHI'] - dict_mkr_coords['RVKN'])
    i_vect = i_vect / np.linalg.norm(i_vect, axis=1)[:, None]
    j_vect = np.cross(i_vect, dict_mkr_coords['RTHI'] - dict_mkr_coords['RVHI'], axis=1)
    j_vect = j_vect / np.linalg.norm(j_vect, axis=1)[:, None]
    k_vect = np.cross(i_vect, j_vect)
    jcs_r_thigh = np.dstack((i_vect, j_vect, k_vect))
