...
class TCNDataGenerator(Sequence):
    ...
    def __init__(self, data_dir, file_names, window_size=32, stride=1, batch_size=32, freq_factor=20, delay=1,
                 ...
                 preproc=_pass, angproc=_pass, ppkwargs=None, gap_windows=None, channel_mask=None):
        'Initialization'
    ...
    def data_generation(self, cur_indexes):
        ...
        X = np.array(
            [self.emg_data[file_id][:, win_id * self.stride:win_id * self.stride + self.window_size].transpose()
             for file_id, win_id in ids])
        Y = np.array(
            [np.squeeze(self.angle_data[file_id][:, win_id + int(self.delay / self.stride), list(self.dims)])
             for file_id, win_id in ids])
